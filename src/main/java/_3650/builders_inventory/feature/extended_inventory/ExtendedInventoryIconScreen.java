...
public class ExtendedInventoryIconScreen extends Screen {
	...
	public void extractRenderState(GuiGraphicsExtractor gui, int mouseX, int mouseY, float partialTick) {
		...
		for (int col = 0; col < 9; col++) {
			int slotX = x + (col * 18);
			ItemStack stack = playerInv.getItem(slot++);
			gui.item(stack, slotX, y);
			gui.itemDecorations(this.font, stack, slotX, y);
		}
		...
		if (this.countSlider != null) this.countSlider.extractRenderState(gui, mouseX, mouseY, partialTick);
		if (this.sizeSlider != null) this.sizeSlider.extractRenderState(gui, mouseX, mouseY, partialTick);
	...
	private void renderTooltip(GuiGraphicsExtractor gui, int x, int y, ItemFindResult hover) {
	...
	void renderTileText(int number, GuiGraphicsExtractor gui, int x, int y) {
		...
		gui.centeredText(this.font, Component.literal(text), 0, 0, 0xFFFFFFFF);
		gui.pose().popMatrix();
	}
	
	void renderTileIcon(ItemStack icon, int iconScaleDown, GuiGraphicsExtractor gui, int x, int y) {
		gui.pose().pushMatrix();
		gui.pose().translate(x, y);
		if (iconScaleDown > 0) {
